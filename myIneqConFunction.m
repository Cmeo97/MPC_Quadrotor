...
global timestep t 
persistent  RX1 RX2 RX3 RY1 RY2 RY3 RZ1 RZ2 RZ3 Theta1 Theta2 Theta3 Theta_pi1  Q cp n r_boundary
...
% This if statement ensures that equal redefinitions are called 
if timestep == t
    
%Definition of Postition [m] of the little wall at the end
cp = [10.5 11;
       5 11;
       0 8];
%sample time [s]          
...
% Ellipsodis orientations [degrees]
...
%Sensitivy parameters of slack variables 
IneqParams.sensitivity.theta_eps=[0.15 0.15 0.15];
IneqParams.sensitivity.theta_pi=[0.1];
IneqParams.Nobs = 3;
...
r_boundary = ones(p,1)*[11 5 8];  % wall position 

% Pre-allocation obstacle positions 
...
% Definition of ellipsoidal obstacles             
...
%Definition future positions of moving obstacles 
frequency = 20/4; 
for i=1:length(X1)
...
RY3(i)  = 10 + 3*sin((i-1)*Ts*frequency + timestep*Ts*frequency);
 ...
 %Sensitivity parameter of boundary slack variable
...
cineq = [ % Obstacles constraints 
         -sqrt(abs(X1.^2*Q(1,1,1) + X2.^2*Q(2,2,1) + X3.^2*Q(3,3,1) + X1.*X2*(Q(1,2,1)+Q(2,1,1)) + ...
         ...
         RX2.*RZ2*(Q(1,3,2) + Q(3,1,2)) + RY2.*RZ2*(Q(2,3,2) + Q(3,2,2)))))  + 2*ones(length(X2),1) - Theta2*e; 
          ...
          cube_ineq  % Cubic obstacles constraints

         -(n(1,:)*(X(1:3,2:p+1)'-r_boundary)')'-Theta_pi1*e;  % Boundary constraints - in this case the wall 
         
         
         Xd1 - 10;   % Velocities constraints 
         Xd2 - 10;
         Xd3 - 10;
         

         U1 - 40;  % Control inputs velocities 
         -U1 ;
         U2 - 40;
         -U2 ; 
         U3 - 40;
